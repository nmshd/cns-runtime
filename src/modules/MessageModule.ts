    ...
    private async handleMessageReceived(messageReceivedEvent: MessageReceivedEvent) {
        ...
        switch (type) {
            case "Mail":
                ...
                event = new MailReceivedEvent(mail, message);
            ...
            case "RequestMail":
                ...
                event = new RequestMailReceivedEvent(requestMail, message);
                this.runtime.eventBus.publish(event);
                this.logger.trace(`Published RequestMailReceivedEvent for ${message.id}`);

                let i = 0;
                for (const request of requestMail.requests) {
                    this.runtime.eventBus.publish(new RequestReceivedEvent(request, message));
        ...
        this.runtime.eventBus.publish(new RelationshipEvent(event, relationship));
