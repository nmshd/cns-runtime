...
import { ConsumptionRequest, ConsumptionRequestSource, ConsumptionResponse, ConsumptionResponseSource, IncomingRequestsController } from "@nmshd/consumption";
...
export class GetIncomingRequestsUseCase extends UseCase<GetIncomingRequestsRequest, ConsumptionRequestDTO[]> {
    ...
    protected async executeInternal(request: GetIncomingRequestsRequest): Promise<Result<ConsumptionRequestDTO[], ApplicationError>> {
        ...
        const consumptionRequests = await this.incomingRequestsController.getIncomingRequests(dbQuery);
