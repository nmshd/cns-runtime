import fs from "fs";
import { CoreServices, FileDTO, OwnerRestriction } from "../../src";
...
describe("File Upload", () => {
    ...
    test("can upload file", async () => {
        const response = await coreServices1.files.uploadOwnFile(await makeUploadRequest());
    ...
    test("uploaded files can be accessed under /Files", async () => {
        ...
        const response = await coreServices1.files.getFiles({ query: { createdAt: file.createdAt } });
        expectSuccess(response);
    ...
    test("uploaded files can be accessed under /Files/Own", async () => {
        ...
        const response = await coreServices1.files.getFiles({
    ...
    test("uploaded files can be accessed under /Files/{id}", async () => {
        ...
        const response = await coreServices1.files.getFile({ id: file.id });
        expectSuccess(response);
    });
    ...
    test("uploaded files keep their size", async () => {
        ...
        const response = await coreServices1.files.downloadFile({ id: file.id });
        expect(response.isSuccess).toBeTruthy();
        expect(response.value.content.byteLength).toStrictEqual(4);
    });

    test("cannot upload an empty file", async () => {
        const response = await coreServices1.files.uploadOwnFile(await makeUploadRequest({ content: Buffer.of() }));
    ...
    test("cannot upload a file that is null", async () => {
        ...
        const response = await coreServices1.files.uploadOwnFile(await makeUploadRequest({ content: null }));
    ...
    test("can upload same file twice", async () => {
        ...
        const response1 = await coreServices1.files.uploadOwnFile(request);
        const response2 = await coreServices1.files.uploadOwnFile(request);
        expectSuccess(response1);
        expectSuccess(response2);
    });

    test("file description is optional", async () => {
        const response = await coreServices1.files.uploadOwnFile(await makeUploadRequest({ description: "" }));
        expectSuccess(response);
    });

    test("cannot upload a file with empty expiry date", async () => {
        const response = await coreServices1.files.uploadOwnFile(await makeUploadRequest({ expiresAt: "" }));
...
describe("Get file", () => {
    ...
    test("accessing not existing file id causes an error", async () => {
        ...
        const response = await coreServices1.files.getFile({ id: notPresentFileId });
...
describe("Upload big files", () => {
    test("can not upload a file that is bigger than 10MB", async () => {
        ...
        const response = await coreServices1.files.uploadOwnFile(await makeUploadRequest({ content: file }));
...
describe("Files query", () => {
    test("files can be queried by their attributes", async () => {
        const file = await uploadFile(coreServices1);
        const conditions = new QueryParamConditions(file, coreServices1)
    ...
    test("peer files can be queried by their attributes", async () => {
        const file = await exchangeFile(coreServices1, coreServices2);
        const conditions = new QueryParamConditions(file, coreServices2)
            .addDateSet("createdAt")
            .addDateSet("createdBy")
            .addDateSet("createdByDevice")
            .addDateSet("deletedAt")
...
describe("Load peer file with token reference", () => {
    ...
    beforeAll(async () => {
        file = await uploadFile(coreServices1);
    });

    test("before the peer file is loaded another client cannot access it", async () => {
        expect(file).toBeDefined();

        const response = await coreServices2.files.getFile({ id: file.id });
        expectError(response, "File not found. Make sure the ID exists and the record is not expired.", "error.runtime.recordNotFound");
    });
    ...
    test("peer file can be loaded with truncated token reference", async () => {
        ...
        const token = (await coreServices1.files.createTokenForFile({ fileId: file.id })).value;
        const response = await coreServices2.files.loadPeerFile({ reference: token.truncatedReference });
    ...
    test("after peer file is loaded it can be accessed under /Files", async () => {
        ...
        const response = await coreServices2.files.getFiles({ query: { createdAt: file.createdAt } });
        expectSuccess(response);
    ...
    test("passing token id as truncated token reference causes an error", async () => {
        ...
        const response = await coreServices2.files.loadPeerFile({ reference: token.id });
    ...
    test("passing file id as truncated token reference causes an error", async () => {
        ...
        const response = await coreServices2.files.loadPeerFile({ reference: file.id });
        expectError(response, "reference is invalid", "error.runtime.validation.invalidPropertyValue");
    });

    test.each(illegalParameters)("passing %p as truncated token reference causes an error", async (tokenReference) => {
        const response = await coreServices2.files.loadPeerFile({ reference: tokenReference! });
...
describe("Load peer file with file id and secret", () => {
    ...
    test("peer file can be loaded with file id and secret key", async () => {
        ...
        const response = await coreServices2.files.loadPeerFile({ id: file.id, secretKey: file.secretKey });
    ...
    test("passing an unkown file id causes an error", async () => {
        ...
        const response = await coreServices2.files.loadPeerFile({ id: UNKOWN_FILE_ID, secretKey: file.secretKey });
    ...
    test("passing an unkown token id as file id causes an error", async () => {
        ...
        const response = await coreServices2.files.loadPeerFile({ id: UNKOWN_TOKEN_ID, secretKey: file.secretKey });
    ...
    test.each(illegalParameters)("passing valid file id and %p as secret key", async (secretKey) => {
        const response = await coreServices2.files.loadPeerFile({ id: file.id, secretKey: secretKey! });

        expectError(response, "The given combination of properties in the payload is not supported.", "error.runtime.validation.invalidPayload");
    });

    test.each(illegalParameters)("passing %p as file id and valid secret key", async (fileId) => {
        const response = await coreServices2.files.loadPeerFile({ id: fileId!, secretKey: file.secretKey });
    ...
    test.each(combinations(illegalParameters, illegalParameters))("passing %p as file id and %p as secret key", async (fileId, secretKey) => {
        const response = await coreServices2.files.loadPeerFile({ id: fileId!, secretKey: secretKey! });
