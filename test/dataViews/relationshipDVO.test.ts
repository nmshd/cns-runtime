...
describe("RelationshipDVO", () => {
    test("check the relationship dvo for the templator", async () => {
        ...
        expect(dvo.relationship!.id).toStrictEqual(dto.id);
        ...
        expect(change.id).toStrictEqual(dto.changes[0].id);
        expect(change.type).toBe("RelationshipChangeDVO");
        expect(change.status).toStrictEqual(dto.changes[0].status);
        ...
        expect(change.request.createdAt).toStrictEqual(dto.changes[0].request.createdAt);
        expect(change.request.createdBy).toStrictEqual(dto.changes[0].request.createdBy);
        expect(change.request.createdByDevice).toStrictEqual(dto.changes[0].request.createdByDevice);
        expect(change.request.content).toStrictEqual(dto.changes[0].request.content);
        expect(change.isOwn).toBe(false);
        expect(change.response!.type).toBe("RelationshipChangeResponseDVO");
        expect(change.response!.createdAt).toStrictEqual(dto.changes[0].response!.createdAt);
        expect(change.response!.createdBy).toStrictEqual(dto.changes[0].response!.createdBy);
        expect(change.response!.createdByDevice).toStrictEqual(dto.changes[0].response!.createdByDevice);
        expect(change.response!.content).toStrictEqual(dto.changes[0].response!.content);
        expect(change.date).toStrictEqual(dto.changes[0].response!.createdAt);
    });
